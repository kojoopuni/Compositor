...
/// Select > Subject and Select > Color Range: selections found from the active layer's pixels rather than drawn.
nonisolated enum SmartSelection {
    /// The outline, in `image`'s own pixels, of every pixel of `mask` (grayscale, the same size) at least half white.
    static func outline(ofWhiteIn mask: CGImage) throws -> CGPath? {
        let width = mask.width, height = mask.height
        let context = try BrushRaster.context(width: width, height: height, mask: true)
        BrushRaster.draw(mask, in: CGRect(x: 0, y: 0, width: width, height: height), mask: true, context: context)
        guard let data = context.data else { throw ExportError.render }
        let bytes = data.assumingMemoryBound(to: UInt8.self)
        var selected = [UInt8](repeating: 0, count: width * height)
        for y in 0..<height { for x in 0..<width where bytes[y * context.bytesPerRow + x] >= 128 { selected[y * width + x] = 1 } }
...
extension EditorSession {
    ...
    /// Selects the active layer's foreground subject, found on-device as Remove Background finds it.
    func selectSubject() async {
        guard canSelectFromPixels, let layer = activeLayer, let image = layer.asset?.image else { return }
        isProjectBusy = true
        defer { isProjectBusy = false }
        do {
            let settings = FilterSettings()
            let path = try await Task.detached(priority: .userInitiated) { () -> CGPath? in
                try SmartSelection.outline(ofWhiteIn: try SubjectRemoval.subjectMask(image, under: nil, settings: settings))
            }.value
            try place(path, from: layer, image: image, name: "Select Subject", empty: "No subject was found in this layer.")
